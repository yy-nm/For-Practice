...
/*
* sha256.c: accord to rfc 6234 implements sha256
*
* Authors:
...
int sha256_update(struct sha256_context *context, const char *in, const int len_in)
{
	...
	if (in)
	{
		...
		if (context->ptr != 0 && context->ptr + l >= LEN_BLOCK) {
			...
			_sha256_update(context, context->block, LEN_BLOCK);
			in += cpycount;
			l -= cpycount;
			context->ptr = 0;
		}
...
{
	if (!out)
		return SHA256_OUTPUT_NULL;
	if (len_out < LEN_RESULT)
		return SHA256_LEN_ERROR;

	int ret = sha256_update(context, NULL, 0);
	if (ret != SHA256_OK)
		return ret;
...
int sha256(const char *input, const int len_input, char *output, const int len_output)
{
	...
	sha256_init(&c);
	sha256_update(&c, input, len_input);
	sha256_final(&c, output, len_output);
	return SHA256_OK;
}
