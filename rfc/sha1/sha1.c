...
// date: 2016-05-12 
// desc: use method 1
// sha1 padding length is big-endian
...
void _sha1_update_1(struct sha1_context *context, const char in[LEN_BLOCK])
{
	U32 W[80];
	int i;
	const U8 *M = in;
...
int sha1_update(struct sha1_context *context, const char *in, const int len_in)
{
	...
	if (in) {
		...
		if (context->ptr != 0 && context->ptr + l > LEN_BLOCK) {
			...
			_sha1_update_1(context, context->block);
			in += cpycount;
			l -= cpycount;
			context->ptr = 0;
		}
...
int sha1(const char *input, const int len_input, char *output, const int len_output)
{
	...
	int i;
	for (i = 0; i + LEN_BLOCK <= len_input; i += LEN_BLOCK) {
		sha1_update(&c, input, LEN_BLOCK);
		input += LEN_BLOCK;
	}
	if (i < len_input)
		sha1_update(&c, input, len_input - i);
