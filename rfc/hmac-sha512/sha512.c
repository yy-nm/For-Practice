...
/*
* sha512.c: accord to rfc 6234 implements sha512
...
int sha512_update(struct sha512_context *context, const char *in, const int len_in)
{
	...
	if (in)
	{
		...
		if (context->ptr != 0 && context->ptr + l >= LEN_BLOCK) {
			...
			_sha512_update(context, context->block, LEN_BLOCK);
			in += cpycount;
			l -= cpycount;
			context->ptr = 0;
		}
...
{
	if (!out)
		return SHA512_OUTPUT_NULL;
	if (len_out < LEN_RESULT)
		return SHA512_LEN_ERROR;

	int ret = sha512_update(context, NULL, 0);
	if (ret != SHA512_OK)
		return ret;
...
int sha512(const char *input, const int len_input, char *output, const int len_output)
{
	...
	sha512_init(&c);
	sha512_update(&c, input, len_input);
	sha512_final(&c, output, len_output);
	return SHA512_OK;
}
