...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < testvector_count; i++) {
		ret = hmac_sha512(testvector_key[i], strlen(testvector_key[i]), testvector_data[i], strlen(testvector_data[i]), buf_mid, sizeof(buf_mid));
		if (ret) {
			printf("calc key: \"%s\", data: \"%s\" hmac-sha512 fail, error code: %d\n", "testvector_key[i]", "testvector_data[i]", ret);
			continue;
		}
		tohex(buf_mid, sizeof(buf_mid), buf, sizeof(buf));
		buf[LEN_BUF * 2] = '\0';
		if (strcmp(buf, testvector_result[i]) == 0) {
			printf("calc key: \"%s\", data: \"%s\" hmac-sha512 success, result: \"%s\"\n", "testvector_key[i]", "testvector_data[i]", buf);
		} else {
			printf("calc key: \"%s\", data: \"%s\" hmac-sha512 success, but not match right one: \n\"%s\"\n result: \n\"%s\"\n"
